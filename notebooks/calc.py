...
def regional_zonal_mean(da_in, grid, rmask, rmaskdict, lat_axis, lat_field='TLAT', ydim='nlat', xdim='nlon', area_field='TAREA'):
    ...
    for i in rmaskdict:
        ...
        # Calculate the numerator
        histVolCoordDepth = histogram(lat_2d_masked.broadcast_like(area_masked).where(~np.isnan(area_masked)), bins=[bins], weights=area_masked, dim=[ydim, xdim])
        # Calculate the denominator
        histTVolCoordDepth = histogram(lat_2d_masked.broadcast_like(area_masked).where(~np.isnan(area_masked)), bins=[bins], weights=(area_masked*da_masked).fillna(0), dim=[ydim, xdim])
